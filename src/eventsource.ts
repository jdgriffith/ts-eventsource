...
function EventSource(url: string, eventSourceInitDict: any) {
  ...
  const retryDelayStrategy = new retryDelay.RetryDelayStrategy(
  ...
  function makeRequestOptions() {
    ...
    if (config.https) {
      for (const optName in config.https) {
        if (httpsOptions.indexOf(optName) === -1) {
          continue
        }

        const option = config.https[optName]
        if (option !== undefined) {
  ...
  function failed(error) {
    ...
    const shouldRetry = (config.errorFilter || defaultErrorFilter)(errorEvent)
    if (shouldRetry) {
...
for (const i in supportedOptions) {
